...
class CVDDTrainer(BaseTrainer):
    ...
    def __init__(self, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 150, lr_milestones: tuple = (),
        ...
        self.test_auc = 0.0
        self.test_aupr_in = 0.0
        self.test_aupr_out = 0.0
    ...
    def test(self, dataset: BaseADDataset, net: CVDDNet, ad_score='context_dist_mean'):
        ...
        if np.sum(labels) > 0:
            ...
            if ad_score == 'context_dist_mean':
                ...
                self.test_aupr_in = auc(recall, precision)

                precision, recall, _ = precision_recall_curve(labels, -scores, pos_label=0)
                self.test_aupr_out = auc(recall, precision)
            if ad_score == 'context_best':
                ...
                for context in range(n_attention_heads):
                    ...
                    precision, recall, _ = precision_recall_curve(labels, self.test_dists[:, context])
                    aupr_in_candidate = auc(recall, precision)
                    print(aupr_in_candidate)
                    if aupr_in_candidate > self.test_aupr_in:
                        self.test_auc = aupr_in_candidate
                        best_context = context
                    else:
                        pass

                    precision, recall, _ = precision_recall_curve(labels, -self.test_dists[:, context], pos_label=0)
                    aupr_out_candidate = auc(recall, precision)
                    print(aupr_out_candidate)
                    if aupr_out_candidate > self.test_aupr_out:
                        self.test_auc = aupr_out_candidate
                        best_context = context
                    else:
                        pass
        else:
        ...
        logger.info('Test AUPR-In: {:.2f}%'.format(100. * self.test_aupr_in))
        logger.info('Test AUPR-Out: {:.2f}%'.format(100. * self.test_aupr_out))
