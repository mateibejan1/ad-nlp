...
class IsoForest(object):
    ...
    def __init__(self, n_estimators=100, max_samples='auto', contamination='auto', n_jobs=-1):
        """Init OCSVM instance."""
        ...
        self.model = IsolationForest(n_estimators=n_estimators, max_samples=max_samples, contamination=contamination, n_jobs=n_jobs)
        self.embedding = None
    ...
    def test(self, dataset: BaseADDataset, device: str = 'cpu', n_jobs_dataloader: int = 0):
        ...
        self.results['test_aupr_in'] = auc(recall, precision)

        precision, recall, _ = precision_recall_curve(labels, -scores, pos_label=0)
        self.results['test_aupr_out'] = auc(recall, precision)

        # Log results
        logger.info('Test AUC: {:.2f}%'.format(100. * self.results['test_auc']))
        logger.info('Test AUPR-In: {:.2f}%'.format(100. * self.results['test_aupr_in']))
        logger.info('Test AUPR-Out: {:.2f}%'.format(100. * self.results['test_aupr_out']))
