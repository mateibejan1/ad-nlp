...
class PAN_Dataset(TorchnlpDataset):
    ...
    def __init__(self, root: str, normal_class=0, tokenizer='spacy', use_tfidf_weights=False, append_sos=False,
        ...
        self.n_classes = 2
        classes = ['2959814', '26060', '2140386', '153679', '767304', '1722748', '2222085', '6920610', '4653911', '1925250', '2542016', '1601179', '5520165', '2138485', '2347643', '1767575', '4689973', '1174083', '2974486', '243571', '535991', '292499', '1449725', '4098044', '1193674', '1440309', '1713618', '1506279', '3661217', '775169']

        self.normal_classes = [classes[normal_class]]
        del classes[normal_class]
        self.outlier_class = classes

        self.train_set, self.test_set = agnews_dataset(directory='../../PAN2020', train=True, test=True)
        ...
        for i, row in enumerate(self.test_set):
            row['author'] = row['label']
            row['text_id'] = row['text_id']
...
def agnews_dataset(directory='../../PAN2020', train=True, test=False):
    classes = ['2959814', '26060', '2140386', '153679', '767304', '1722748', '2222085', '6920610', '4653911', '1925250', '2542016', '1601179', '5520165', '2138485', '2347643', '1767575', '4689973', '1174083', '2974486', '243571', '535991', '292499', '1449725', '4098044', '1193674', '1440309', '1713618', '1506279', '3661217', '775169']
    ret = []
    splits = [split_set for (requested, split_set) in [(train, 'train_new'), (test, 'test_new')] if requested]
    ...
    for split_set in splits:
        ...
        if split_set == 'train_new':
            for subset in classes:
                with open(f'{path}/baselines/{subset}.txt') as fp:
                # with open(f'{path}/{subset}.txt') as fp:
                    ...
                    # print(f'[train]{path}/baselines/{subset}.txt')
                    line = fp.readline()
                    while line:
                        if line != '\n':
                            examples.append({
            ...
            for subset in classes:
                with open(f'{path}/{subset}.txt') as fp:
                    ...
                    while line:
                        if line != '\n':
                            line = line[:-1]
                            chunks = [line[i:i+n_tokens] for i in range(0, len(line), n_tokens)]
                            for chunk in chunks:
                                examples.append({
                                    'text': chunk,
                                    'label': subset,
                    ...
                    # print(f'[{subset}]Total texts: {sample_no}')
